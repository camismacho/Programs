...
def CreatePlayers(size):
    ...
    #starttime = time.time()
    row = int(size)
    File = open( "players" + str(row) + ".txt", "w")
    ...
    for i in range(row):
        ...
        data = (firstName + lastName + comma + playerID + comma + team + comma + position + comma + touchdowns + comma + totalYards + comma + salary)
        File.write(str(data) + "\n")
    #Close the file
    File.close()
